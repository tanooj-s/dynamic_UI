...
class Company:
  ...
  def update_price(self):
    ...
    change = np.random.uniform(low = 0.1, high = 10.1) # share price only changes between 0.1 and 10
    self.share_price += up_down*change
...
def create_client_match_to_company_and_brokerage(tx,client,company,brokerage):
  ...
  tx.run('MATCH (client:Client{id:$client_id}) WITH client OPTIONAL MATCH (company:Company{id:$company_id}) WITH client, company MERGE (client)-[:works_for{employed_as:$designation}]->(company)', # added employed_as attribute for designation to works_for to identifiy kmp (instead of having 3 different relationship types)
        client_id=client.id, company_id=company.id, designation=client.designation)
...
def create_and_match_trade(tx, trade, client, company, timestamp):
  tx.run('CREATE (t:Trade{type:$type, exchange:$exchange, ISIN:$ISIN, volume:$volume, timestamp:$timestamp, share_price:$share_price})',
          type=trade.type, exchange=trade.exchange, ISIN=trade.ISIN, volume=trade.volume, timestamp=timestamp, share_price=trade.share_price)
